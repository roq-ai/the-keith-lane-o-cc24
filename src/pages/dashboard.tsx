...
import { getDueDiligences } from 'apiSdk/due-diligences';
import { DueDiligenceInterface } from 'interfaces/due-diligence';
import { getFinancePartners } from 'apiSdk/finance-partners';
import { FinancePartnerInterface } from 'interfaces/finance-partner';
import { getFundingProviders } from 'apiSdk/funding-providers';
import { FundingProviderInterface } from 'interfaces/funding-provider';

import { FundingProviderListPage as ShowCaseList } from 'pages/funding-providers';

function DashboardPage() {
  const { session } = useSession();

  const dueDiligenceStat = useSWR<PaginatedInterface<DueDiligenceInterface>>(
    () => `/due-diligences`,
    () => getDueDiligences({ limit: 1, offset: 0 }),
  );

  const financePartnerStat = useSWR<PaginatedInterface<FinancePartnerInterface>>(
    () => `/finance-partners`,
    () => getFinancePartners({ limit: 1, offset: 0 }),
  );

  const fundingProviderStat = useSWR<PaginatedInterface<FundingProviderInterface>>(
    () => `/funding-providers`,
    () => getFundingProviders({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Due Diligences',
      result: dueDiligenceStat?.data?.totalCount,
      isLoading: dueDiligenceStat.isLoading,
    },
    {
      title: 'Finance Partners',
      result: financePartnerStat?.data?.totalCount,
      isLoading: financePartnerStat.isLoading,
    },
    {
      title: 'Funding Providers',
      result: fundingProviderStat?.data?.totalCount,
      isLoading: fundingProviderStat.isLoading,
    },
  ];
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Funding Providers
                </Text>
                <NextLink href={`funding-providers`} passHref legacyBehavior>
                  <Button
