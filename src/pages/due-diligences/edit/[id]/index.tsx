...
import { getDueDiligenceById, updateDueDiligenceById } from 'apiSdk/due-diligences';
...
function DueDiligenceEditPage() {
  ...
  const handleSubmit = async (values: DueDiligenceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateDueDiligenceById(id, values);
      mutate(updated);
      resetForm();
