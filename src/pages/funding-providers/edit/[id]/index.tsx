...
import { getFundingProviderById, updateFundingProviderById } from 'apiSdk/funding-providers';
...
function FundingProviderEditPage() {
  const router = useRouter();
  const id = router.query.id as string;

  const { data, error, isLoading, mutate } = useSWR<FundingProviderInterface>(
    () => (id ? `/funding-providers/${id}` : null),
    () => getFundingProviderById(id),
  );
  ...
  const handleSubmit = async (values: FundingProviderInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateFundingProviderById(id, values);
      mutate(updated);
      resetForm();
